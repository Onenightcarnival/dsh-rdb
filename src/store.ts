...
import type { DbKind, DbProfile, DbProfilePayload, DbProfileSummary, RdbSettings } from './protocol.ts'
...
export function validateProfile(payload: DbProfilePayload): string | undefined {
  ...
  if (payload.kind === 'sqlite') {
    ...
    if ((str(payload.host)?.trim() ?? '') === '') return 'host is required'
...
function defaultName(payload: DbProfilePayload): string {
  ...
  return `${(payload.database ?? '').trim()}@${(payload.host ?? '').trim()}`
}
...
export class ProfileStore {
  ...
  create(payload: DbProfilePayload): DbProfile {
    ...
    const profile: DbProfile = {
      ...
      host: kind === 'sqlite' ? '' : (payload.host ?? '').trim(),
  ...
  update(id: string, patch: DbProfilePayload): DbProfile {
    ...
    profile.host = kind === 'sqlite' ? '' : (merged.host ?? '').trim()
